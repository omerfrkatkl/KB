...
- **Host:** WSL2 Ubuntu. Python 3.12. `uv` for env + lockfile; `ruff` (lint + format); `pytest`. `make check` = ruff + pytest and is the gate for every work package.
...
- **Vendored tools (`tools/`):** `typst` release binary, exact version pinned in `tools/VERSIONS` with sha256, fetched by a bootstrap script; same for fonts (`fonts/`: Fira Sans, Fira Math from official repos, shas pinned). Builds always use `--font-path fonts/` and the vendored binary — zero dependence on system state, reproducible for life.
- **Claude Code:** installed and logged in inside WSL (your existing install). The pipeline shells out to it; it is never a Python dependency.
...
5. **Capture groups (`groups.py`)** — generalizes the former board-only "session" so continuation (§7.3) works for every source kind:
   - **board × photo** — session identity resolved by explicit precedence, because upload timestamps demonstrably do not carry it (docs/CAPTURE-CORPUS-ANALYSIS.md, Finding 1: one real upload batch merged five lectures inside fifteen minutes):
