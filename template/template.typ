...
// ═════════════════════════════════════════════════
#let thm-box(name, number, body, title: none, accent: black, bg: white, icon: "•") = {
  v(0.6em)
    ...
    block(inset: 0pt, outset: 0pt, below: 0pt, above: 0pt, width: 100%, breakable: false, {
      box(
        ...
        text(fill: white, weight: "bold", size: 9.5pt, font: heading-font)[
          #icon #h(2pt) #smallcaps(name) #number
...
// ═════════════════════════════════════════════════
#let semi-box(name, number, body, title: none, accent: black, icon: "•") = {
  v(0.6em)
  block(breakable: true, width: 100%, below: 1em, above: 0pt, {
    block(inset: 0pt, outset: 0pt, below: 0pt, above: 0pt, width: 100%, breakable: false, {
      box(
...
// ═════════════════════════════════════════════════
#let light-box(name, number, body, title: none, accent: black, icon: "•") = {
  v(0.4em)
  block(
    ...
    breakable: true,
    {
      text(fill: accent, weight: "bold", size: 10.5pt, font: heading-font)[#icon #h(1pt) #name]
      if number != "" {
...
#let project(
  ...
  show figure.where(kind: "math-env"): it => {
    ...
    if supp in boxed-envs {
      thm-box(supp, number, it.body, title: env-title, accent: p.accent, bg: p.bg, icon: p.icon)
    } else if supp in semi-envs {
      semi-box(supp, number, it.body, title: env-title, accent: p.accent, icon: p.icon)
    } else {
      light-box(supp, number, it.body, title: env-title, accent: p.accent, icon: p.icon)
    }
  }
...
#let math-item(body, title, supplement, key, nl: false) = [
  #context {
    let h = counter(heading).get()
    let section = if h.len() > 0 { h.first() } else { 0 }
...
#let math-item-unnumbered(body, title, supplement, nl: false) = figure(
  if nl { parbreak() + body } else { body },
  kind: "math-env",
  supplement: supplement,
  caption: _title-content(title),
...
// ═════════════════════════════════════════════════
#let thm(body, title: none, nl: false) = math-item(body, title, "Theorem", "thm", nl: nl)
#let def(body, title: none, nl: false) = math-item(body, title, "Definition", "def", nl: nl)
#let lem(body, title: none, nl: false) = math-item(body, title, "Lemma", "lem", nl: nl)
#let prop(body, title: none, nl: false) = math-item(body, title, "Proposition", "prop", nl: nl)
#let cor(body, title: none, nl: false) = math-item(body, title, "Corollary", "cor", nl: nl)
#let axiom(body, title: none, nl: false) = math-item(body, title, "Axiom", "axiom", nl: nl)
#let postulate(body, title: none, nl: false) = math-item(body, title, "Postulate", "postulate", nl: nl)
#let conjecture(body, title: none, nl: false) = math-item(body, title, "Conjecture", "conjecture", nl: nl)
#let claim(body, title: none, nl: false) = math-item(body, title, "Claim", "claim", nl: nl)

// ═════════════════════════════════════════════════
// TIER 2 & 3 — unnumbered environments
// ═════════════════════════════════════════════════
#let recall(body, title: none, nl: false) = math-item-unnumbered(body, title, "Recall", nl: nl)
#let remark(body, title: none, nl: false) = math-item-unnumbered(body, title, "Remark", nl: nl)
#let ex(body, title: none, nl: false) = math-item-unnumbered(body, title, "Example", nl: nl)
#let notation(body, title: none, nl: false) = math-item-unnumbered(body, title, "Notation", nl: nl)
